...
public class PrimeGenerator {
   ...
   public BigInteger getStrongPrime() {
	  ...
	  BigInteger r;
	  do {
		 r=new BigInteger("2").multiply(i).multiply(t).add(BigInteger.ONE);
		 i=i.add(BigInteger.ONE);
	  } while (!r.isProbablePrime(certainty));
	  BigInteger z=s.modPow(r.subtract(new BigInteger("2")),r);
	  BigInteger pstar=new BigInteger("2").multiply(z).multiply(s).subtract(BigInteger.ONE);
	  BigInteger k=BigInteger.valueOf(1);
	  //The strong prime p is the first prime in the sequence 2rs+p*, 2*2rs+p*, 2*3rs+p*,...
	  BigInteger p=new BigInteger("2").multiply(r).multiply(s).add(pstar);
	  while (p.bitLength()<=minBitLength) {
		 k=k.multiply(new BigInteger("2"));
		 p=new BigInteger("2").multiply(k).multiply(r).multiply(s).add(pstar);
	  }
   ...
   public BigInteger getSafePrime() {
	  ...
	  do {
		 ...
		 p=new BigInteger("2").multiply(r).multiply(t).add(BigInteger.ONE);
	  } while (!p.isProbablePrime(certainty));
	  return p;
   }
	  ...
	  //otherwise, find the factors of r and add them to the Vector
		 ...
		 while (square.compareTo(r)<=0) {
			...
			if (r.mod(divisor).equals(BigInteger.ZERO)) {
			   ...
			   while (r.mod(divisor).equals(BigInteger.ZERO)) r=r.divide(divisor);
			}
